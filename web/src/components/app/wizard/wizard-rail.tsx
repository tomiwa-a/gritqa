...
/** Any wizard's steps, so the rail is not tied to onboarding's three. */
export type WizardStep = { key: string; label: string; hint: string };
...
export function stepHref(key: string, os?: string) {
...
export function WizardRail({
  ...
  os?: string;
  steps?: WizardStep[];
  hrefFor?: (key: string, os?: string) => string;
  label?: string;
  /** `auto` is the page rail — a column on wide screens. `row` stays a strip, for overlays. */
  layout?: 'auto' | 'row';
}) {
  ...
  return (
    <nav aria-label={label} className={cn(!row && 'lg:sticky lg:top-6')}>
      <ol className={cn(row ? 'hidden' : 'hidden lg:flex lg:flex-col')}>
        {steps.map((step, i) => {
          const done = i < index;
          const on = i === index;
      ...
      <ol className={cn('flex gap-1.5', !row && 'lg:hidden')}>
        {steps.map((step, i) => (
